<x-layout>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    <!-- Job Listing 1 -->
    <div class="rounded-lg shadow-md bg-white p-4">
        <div class="flex items-center space-between gap-4">
            <img
                src="/images/logos/logo-algorix.png"
                alt=""
                class="w-14"
            />
            <div>
                <h2 class="text-xl font-semibold">
                    Software Engineer
                </h2>
                <p class="text-sm text-gray-500">Full-Time</p>
            </div>
        </div>
        <p class="text-gray-700 text-lg mt-2">
            Design and develop high-quality software applications,
            collaborating with teams and ensuring efficient
            solutions.
        </p>
        <ul class="my-4 bg-gray-100 p-4 rounded">
            <li class="mb-2"><strong>Salary:</strong> $90,000</li>
            <li class="mb-2">
                <strong>Location:</strong> Albany, NY
                <span
                    class="text-xs bg-red-500 text-white rounded-full px-2 py-1 ml-2"
                    >On-site</span
                >
            </li>
            <li class="mb-2">
                <strong>Tags:</strong> <span>Development</span>,
                <span>Coding</span>, <span>Java</span>,
                <span>Python</span>
            </li>
        </ul>
        <a
            href="jobs/show"
            class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200"
        >
            Details
        </a>
    </div>

    <!-- Job Listing 2 -->
    <div class="rounded-lg shadow-md bg-white p-4">
        <div class="flex items-center space-between gap-4">
            <img
                src="/images/logos/logo-bitwave.png"
                alt=""
                class="w-14"
            />
            <div>
                <h2 class="text-xl font-semibold">
                    Marketing Specialist
                </h2>
                <p class="text-sm text-gray-500">Full-Time</p>
            </div>
        </div>
        <p class="text-gray-700 text-lg mt-2">
            Develop and execute marketing campaigns, conduct market
            research, and drive brand engagement.
        </p>
        <ul class="my-4 bg-gray-100 p-4 rounded">
            <li class="mb-2"><strong>Salary:</strong> $70,000</li>
            <li class="mb-2">
                <strong>Location:</strong> San Francisco, CA
                <span
                    class="text-xs bg-green-500 text-white rounded-full px-2 py-1 ml-2"
                    >Remote</span
                >
            </li>
            <li class="mb-2">
                <strong>Tags:</strong> <span>Marketing</span>,
                <span>Advertising</span>
            </li>
        </ul>
        <a
            href="jobs/show"
            class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200"
        >
            Details
        </a>
    </div>

    <!-- Job Listing 3 -->
    <div class="rounded-lg shadow-md bg-white p-4">
        <div class="flex items-center space-between gap-4">
            <img
                src="/images/logos/logo-nextgen.png"
                alt=""
                class="w-14"
            />
            <div>
                <h2 class="text-xl font-semibold">Web Developer</h2>
                <p class="text-sm text-gray-500">Contract</p>
            </div>
        </div>
        <p class="text-gray-700 text-lg mt-2">
            Build and maintain web applications, handling full
            development lifecycle from design to deployment.
        </p>
        <ul class="my-4 bg-gray-100 p-4 rounded">
            <li class="mb-2"><strong>Salary:</strong> $85,000</li>
            <li class="mb-2">
                <strong>Location:</strong> Chicago, IL
                <span
                    class="text-xs bg-red-500 text-white rounded-full px-2 py-1 ml-2"
                    >On-site</span
                >
            </li>
            <li class="mb-2">
                <strong>Tags:</strong>
                <span>Web Development</span>,
                <span>Programming</span>
            </li>
        </ul>
        <a
            href="jobs/show"
            class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200"
        >
            Details
        </a>
    </div>

    <!-- Job Listing 4 -->
    <div class="rounded-lg shadow-md bg-white p-4">
        <div class="flex items-center space-between gap-4">
            <img
                src="/images/logos/logo-quantumcode.png"
                alt=""
                class="w-14"
            />
            <div>
                <h2 class="text-xl font-semibold">Data Analyst</h2>
                <p class="text-sm text-gray-500">Full-Time</p>
            </div>
        </div>
        <p class="text-gray-700 text-lg mt-2">
            Analyze complex data to derive insights and support
            data-driven decision-making processes.
        </p>
        <ul class="my-4 bg-gray-100 p-4 rounded">
            <li class="mb-2"><strong>Salary:</strong> $75,000</li>
            <li class="mb-2">
                <strong>Location:</strong> Chicago, IL
                <span
                    class="text-xs bg-red-500 text-white rounded-full px-2 py-1 ml-2"
                    >On-site</span
                >
            </li>
            <li class="mb-2">
                <strong>Tags:</strong>
                <span>Data Analysis</span>,
                <span>Statistics</span>
            </li>
        </ul>
        <a
            href="jobs/show"
            class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200"
        >
            Details
        </a>
    </div>

    <!-- Job Listing 5 -->
    <div class="rounded-lg shadow-md bg-white p-4">
        <div class="flex items-center space-between gap-4">
            <img
                src="/images/logos/logo-shield.png"
                alt=""
                class="w-14"
            />
            <div>
                <h2 class="text-xl font-semibold">
                    Graphic Designer
                </h2>
                <p class="text-sm text-gray-500">Full-Time</p>
            </div>
        </div>
        <p class="text-gray-700 text-lg mt-2">
            Create compelling designs across various media, from
            branding to digital, while collaborating with the
            creative team.
        </p>
        <ul class="my-4 bg-gray-100 p-4 rounded">
            <li class="mb-2"><strong>Salary:</strong> $70,000</li>
            <li class="mb-2">
                <strong>Location:</strong> Albany, NY
                <span
                    class="text-xs bg-red-500 text-white rounded-full px-2 py-1 ml-2"
                    >On-site</span
                >
            </li>
            <li class="mb-2">
                <strong>Tags:</strong>
                <span>Graphic Design</span>,
                <span>Creative</span>
            </li>
        </ul>
        <a
            href="jobs/show"
            class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200"
        >
            Details
        </a>
    </div>

    <!-- Job Listing 6 -->
    <div class="rounded-lg shadow-md bg-white p-4">
        <div class="flex items-center space-between gap-4">
            <img
                src="/images/logos/logo-sparkle.png"
                alt=""
                class="w-14"
            />
            <div>
                <h2 class="text-xl font-semibold">
                    Data Scientist
                </h2>
                <p class="text-sm text-gray-500">Full-Time</p>
            </div>
        </div>
        <p class="text-gray-700 text-lg mt-2">
            Analyze and interpret complex data, build predictive
            models, and solve real-world problems with machine
            learning.
        </p>
        <ul class="my-4 bg-gray-100 p-4 rounded">
            <li class="mb-2"><strong>Salary:</strong> $100,000</li>
            <li class="mb-2">
                <strong>Location:</strong> Boston, MA
                <span
                    class="text-xs bg-green-500 text-white rounded-full px-2 py-1 ml-2"
                    >Remote</span
                >
            </li>
            <li class="mb-2">
                <strong>Tags:</strong>
                <span>Data Science</span>,
                <span>Machine Learning</span>
            </li>
        </ul>
        <a
            href="jobs/show"
            class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200"
        >
            Details
        </a>
    </div>

    <!-- Job Listing 7 -->
    <div class="rounded-lg shadow-md bg-white p-4">
        <div class="flex items-center space-between gap-4">
            <img
                src="/images/logos/logo-vencom.png"
                alt=""
                class="w-14"
            />
            <div>
                <h2 class="text-xl font-semibold">UX Designer</h2>
                <p class="text-sm text-gray-500">Part-Time</p>
            </div>
        </div>
        <p class="text-gray-700 text-lg mt-2">
            Enhance user experience through design, user research,
            and collaboration with developers and product managers.
        </p>
        <ul class="my-4 bg-gray-100 p-4 rounded">
            <li class="mb-2"><strong>Salary:</strong> $80,000</li>
            <li class="mb-2">
                <strong>Location:</strong> San Jose, CA
                <span
                    class="text-xs bg-green-500 text-white rounded-full px-2 py-1 ml-2"
                    >Remote</span
                >
            </li>
            <li class="mb-2">
                <strong>Tags:</strong> <span>UX Design</span>,
                <span>User Research</span>
            </li>
        </ul>
        <a
            href="jobs/show"
            class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200"
        >
            Details
        </a>
    </div>
</div>
<!-- Pagination -->
<div class="flex justify-center mt-6">
    <span class="px-4 py-2 bg-gray-300 text-gray-500 rounded-l-lg"
        >Previous</span
    >
    <a
        href="#"
        class="px-4 py-2 bg-blue-500 text-white hover:bg-blue-600"
        >1</a
    >
    <a
        href="#"
        class="px-4 py-2 bg-gray-300 text-gray-500 hover:bg-blue-600 hover:text-white"
        >2</a
    >
    <a
        href="#"
        class="px-4 py-2 bg-gray-300 text-gray-500 hover:bg-blue-600 hover:text-white"
        >3</a
    >
    <a
        href="#"
        class="px-4 py-2 bg-blue-500 text-white rounded-r-lg hover:bg-blue-600"
        >Next</a
    >
</div>
</x-layout> 