<header class="bg-blue-900 text-white p-4">
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav class="hidden md:flex items-center space-x-4">
            <x-nav-link href="{{url('listings')}}" :active="request()->is('listings')">All Jobs</x-nav-link>
            <x-nav-link href="{{url('listings/saved')}}" :active="request()->is('listings/saved')">Saved Jobs</x-nav-link>
            ...
            <x-button-link href="{{url('listings/create')}}" :active="request()->is('listings/create')" icon="edit">Create Job</x-button-link>
        </nav>
    ...
    <div x-data id="mobile-menu" :class="$store.menu.open ? 'block md:hidden bg-blue-900 text-white mt-5 pb-4 space-y-2' : 'hidden'">
        <x-mobile-nav-link href="{{url('listings')}}" :active="request()->is('listings')">All Jobs</x-mobile-nav-link>
        <x-mobile-nav-link href="{{url('listings/saved')}}" :active="request()->is('listings/saved')">Saved Jobs</x-mobile-nav-link>
        ...
        <x-button-link href="{{url('listings/create')}}" :active="request()->is('listings/create')" icon="edit" class="block px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-black">Create Job</x-button-link>
    </div>
</header>